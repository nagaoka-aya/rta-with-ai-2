...
sequenceDiagram
    ...
    ProjectCreateController ->> ProjectCreateViewHelper: (6) getDivisionOptions()
    ProjectCreateViewHelper -->> ProjectCreateController: (7) List<SelectOption>
    
    ProjectCreateController ->> CodeHelper: (8) getSelectOptions("C0200001", "1")
    CodeHelper -->> ProjectCreateController: (9) List<SelectOption> for projectType
    
    ProjectCreateController ->> CodeHelper: (10) getRadioOptions("C0300001", "1")
    CodeHelper -->> ProjectCreateController: (11) List<RadioOption> for projectClass
    
    ProjectCreateController ->> View: (12) render with model
...
sequenceDiagram
    ...
    alt validation error
        ...
        ProjectCreateController ->> ProjectService: (6) getOrganizationName(organizationId)
        ...
        ProjectService -->> ProjectCreateController: (9) organizationName
        
        ProjectCreateController ->> ProjectService: (10) getClientName(clientId)
        ProjectService ->> ExternalClientAPI: (11) getClientDetail(clientId)
        ExternalClientAPI -->> ProjectService: (12) ClientDetail
        ProjectService -->> ProjectCreateController: (13) clientName
        
        ProjectCreateController ->> View: (14) render confirm page with model
        View -->> Client: (15) プロジェクト登録確認画面
    end
```
...
- **目的:** 初期表示イベントのシーケンス図の(6)〜(7)を実装。画面表示に必要な補助メソッドを提供する
- **内容:**
  - `ProjectCreateViewHelper`クラスを作成する
  - `getDivisionOptions()`メソッドを実装し、事業部のSelectOptionリストを作成する
  - `getProjectTypeOptions()`、`getProjectClassOptions()`メソッドを実装する
  - `getOrganizationName()`、`getClientName()`メソッドを実装する
- **活用するクラス・メソッド:**
...
- **関連ファイル:** `CodeHelper.java`、`ProjectService.java`
...
- **目的:** 確認イベントのシーケンス図の(1)〜(15)を実装。入力内容のバリデーションと確認画面への遷移を行う
- **内容:**
